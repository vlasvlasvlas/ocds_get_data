...
for origen in listaJson2022:
    ...
    with ZipFile(io.BytesIO(response.content)) as zip_file:
        ...
        dfs = {text_file.filename: pd.read_csv(zip_file.open(text_file.filename), quotechar='"')
        ...
        for df in dfs.keys():
            ...
            try:
                print("table name:")
                tablename = 'tmp_oncae_source'+str(df).replace('/','_').replace('.csv','').lower()

                print(tablename)
                # clean
                dfs[df].columns = dfs[df].columns.str.strip().str.lower().str.replace(' ', '_').str.replace('(', '').str.replace(')', '').str.replace('/', '_')
                
                # delimiter chars
                dfs[df] = dfs[df].replace('\\t','', regex=True).replace('|','', regex=True).replace(';','', regex=True)

                # quote chars
                dfs[df] = dfs[df].replace('#','', regex=True).replace('~','', regex=True)
