                ...
                for line in fp.readlines()[desde:hasta]:
                    ...
                    if is_json(line):
                        ...
                        if len(str(compiled_release)) < 50000:
                            ...
                            dfnew = pd.DataFrame(
                                list(flatsplode(compiled_release))
                            ).replace("\\t", " ", regex=True)

                            # columns
                            dfnew.columns = (
                                dfnew.columns.str.strip()
                                .str.lower()
                            ...
                            # quote chars
                            dfnew = dfnew.replace("#", "", regex=True).replace(
                                "~", "", regex=True
                            )

                            # to dfmain
                            dfmain = pd.concat(
                                [dfmain, dfnew], axis=0, ignore_index=True
                            )
